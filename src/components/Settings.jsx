...
const Type = ({ type, title }) => {
  ...
  const { t } = useI18n();
  const settingsItems = type({ options, updateOption, t });
...
const Setting = ({ setting }) => {
  ...
  return (
    <Container>
      {setting === 'Privacy' && <Type type={() => privSettings} title={t('settings.privacy')} />}
      {setting === 'Customize' && <Type type={settings.customizeConfig} title={t('settings.customize')} />}
      {setting === 'Browsing' && <Type type={settings.browsingConfig} title={t('settings.browsing')} />}
      {setting === 'Advanced' && <Type type={settings.advancedConfig} title={t('settings.advanced')} />}
    </Container>
  );
};
