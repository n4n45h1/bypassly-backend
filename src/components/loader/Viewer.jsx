...
import { applyCustomCss } from './CssPanel';
    ...
    tabs.forEach((tab) => {
      ...
      const handleLoad = () => {
        ...
        try {
          ...
          // Inject custom CSS
          applyCustomCss(iframe);
          // Record in browsing history
          const curHref = iframe.contentWindow?.location?.href;
          const curTitle = iframe.contentWindow?.document?.title;
          if (curHref && curHref !== 'about:blank') {
            addHistory(curHref, curTitle, options);
          }
        } catch {}
      };
      const checkState = () => {
      ...
      tabs.forEach((tab) => {
        ...
        try {
          ...
          if (!tab.isLoading && curURL !== prevURL.current[tab.id] && curURL !== tab.url) {
            ...
            addHistory(curURL, curTTL, options);
          }
          if (curTTL && curTTL !== prevTitle.current[tab.id] && curTTL !== tab.title) {
            prevTitle.current[tab.id] = curTTL;
            updateTitle(tab.id, curTTL);
